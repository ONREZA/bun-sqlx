...
`bun-sqlx` is a TypeScript library that ports the developer experience of Rust's `sqlx` to Bun + PostgreSQL: you write raw SQL, a `prepare` step validates it against a live database and emits typed declarations. The library has both a CLI (`bin/bun-sqlx.ts`) and a runtime (`src/index.ts`).

The library is **PostgreSQL-only**, **Bun-only**, and **compile-time-only by design** — no runtime SQL parsing, no runtime validation, no ORM layer.
...
.
├── bin/
│   └── bun-sqlx.ts           CLI entry point (prepare, migrate, watch)
├── src/
│   ├── index.ts              Public package entry (sql, migrate, types)
│   ├── runtime.ts            Bun.SQL wrapper + key renaming + migrate()
│   ├── cache.ts              .bun-sqlx/<fingerprint>.json reader/writer
│   ├── codegen.ts            Emits bun-sqlx.d.ts from CacheEntry[]
│   ├── config.ts             Loads bun-sqlx.config.ts (jsonbTypes map)
│   ├── commands/
...
1. **Scan** (`src/scan/scanner.ts`) — TypeScript AST walk over `.ts` / `.tsx` files. Finds every `sql("literal", ...args)` call where `sql` is imported from `bun-sqlx`. Refuses non-literal first arguments.
...
7. **Persistence** — writes `.bun-sqlx/<fingerprint>.json` per query and emits `bun-sqlx.d.ts` with a `declare module "bun-sqlx" { interface KnownQueries { ... } }` block.
...
The runtime (`src/index.ts` + `src/runtime.ts`) is a thin layer over `Bun.SQL.unsafe(query, params)`. Strict typing comes from a single overload keyed on `keyof KnownQueries` — TypeScript narrows the first string literal argument to a known query, then resolves `params` and the result row type from the registered entry.
...
bun bin/bun-sqlx.ts prepare    --root example
bunx tsc -p example
```
...
- **No runtime dependencies beyond `libpg-query`.** The wire client and everything else are first-party.
...
- `Bun.SQL` returns `int8` as a string by default. The runtime wrapper explicitly sets `{ bigint: true }` so types and runtime agree. Don't remove this without re-checking every `bigint` site.
...
2. `example/` — a complete working setup. Run `prepare`, look at the generated `bun-sqlx.d.ts`.
